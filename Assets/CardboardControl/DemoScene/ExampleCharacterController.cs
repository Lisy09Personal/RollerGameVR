...
public class ExampleCharacterController : MonoBehaviour {
  ...
  */
	void Update() {
		...
		Color newColor;
		if (cardboard.trigger.IsUp) {
			newColor = Color.green;
		} else {
			newColor = Color.red;
		}
		GameObject objIsUp = GameObject.Find("SphereIsUp");
		objIsUp.GetComponent<Renderer>().material.color = newColor;
		if (cardboard.trigger.IsDown) {
			newColor = Color.green;
		} else {
			newColor = Color.red;
		}
